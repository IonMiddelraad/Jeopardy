  ...
  <div>
    <form @submit.prevent="submitCategory" class="space-y-4">
      <div>
        ...
        <input v-model="newCategory.name" type="text" class="w-full border rounded p-2" />
      </div>

      <div v-for="(card, index) in newCategory.cards" :key="index" class="border p-3 rounded bg-gray-50">
        <h3 class="font-semibold mb-2">Question {{ index + 1 }}</h3>
        <input v-model="card.question" placeholder="Question" class="w-full mb-2 p-2 border rounded" />
        <input v-model="card.answer" placeholder="Answer" class="w-full mb-2 p-2 border rounded" />
        <input v-model.number="card.points" type="number" placeholder="Points" class="w-full mb-2 p-2 border rounded" />
