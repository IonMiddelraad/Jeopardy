  ...
  <div class="mx-auto">
      ...
      <div v-for="(category, index) in boardStore.boardData?.categories" :key="index"
        ...
        <div class="flex flex-col gap-2 w-full">
          <p v-for="(card, index) in category.cards" :key="index"
            class="bg-blue-400 text-center p-2 cursor-pointer"
    ...
    <section v-if="chosenCard">
      <div class="flex flex-col items-center mt-4 pt-2 pb-8 mb-4 gap-y-2 ">
        ...
        <button v-if="!showAnswer" class="text-lg font-semibold border-2 border-black rounded-full cursor-pointer px-4 py-2" @click="toggleAnswer(chosenCard)">Reveal Answer</button>

      </div>
    </section>
    <!-- Giving points -->
    <section>
      <div v-if="showAnswer && chosenCard" class="flex flex-row gap-x-8 justify-center mt-8">
